...
void hire_hourly(Database& inDB) {
  ...
  try {
    inDB.add_hourly_employee(firstName, lastName, hourly_rate, hours_worked,
                             in_id);
...
void hire_salaried(Database& inDB) {
  ...
  try {
    inDB.add_salaried_employee(firstName, lastName, salary, hours_worked,
                               in_id);
  } catch (const exception&) {
    cerr << "Unable to add new employee!" << endl;
  }
...
void find_employee_name(Database& inDB) {
  ...
  try {
    shared_ptr<Employee> emp =
        inDB.get_employee(employee_first_name, employee_last_name);
    emp->display();
  } catch (const std::exception&) {
    cerr << "Unable to locate employee!" << endl;
  }
...
void sort_employees(Database& inDB) { inDB.sort_net_pay(); }
